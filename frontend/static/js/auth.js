...
function setupInputSanitization() {
    ...
    const inputConfigs = {
        'nickname': { 
            ...
            errorMsg: 'Nickname can only contain letters, numbers, and underscores (3-20 characters)'
        },
        'firstName': { 
            sanitize: true, 
            validate: validateName,
            errorMsg: 'First name can only contain letters and spaces (2-50 characters)'
        },
        'lastName': { 
            sanitize: true, 
            validate: validateName,
            errorMsg: 'Last name can only contain letters and spaces (2-50 characters)'
        },
        'email': { 
            sanitize: true, 
...
function validateForm() {
    ...
    if (isLoginMode) {
        // Login validation
        const loginIdentifier = sanitizeInput(document.getElementById('loginIdentifier').value);
        ...
        if (!loginIdentifier) {
            ...
            showFieldError('loginIdentifier', 'Please enter a valid email or nickname');
            document.getElementById('loginIdentifier').classList.add('error');
            hasErrors = true;
            if (!firstErrorField) firstErrorField = 'loginIdentifier';
        }
        ...
        const fields = {
            nickname: { value: sanitizeInput(document.getElementById('nickname').value), validator: validateNickname, message: 'Nickname can only contain letters, numbers, and underscores (3-20 characters)' },
            firstName: { value: sanitizeInput(document.getElementById('firstName').value), validator: validateName, message: 'First name can only contain letters and spaces (2-50 characters)' },
            lastName: { value: sanitizeInput(document.getElementById('lastName').value), validator: validateName, message: 'Last name can only contain letters and spaces (2-50 characters)' },
            email: { value: sanitizeInput(document.getElementById('email').value), validator: validateEmail, message: 'Please enter a valid email address' },
            age: { value: document.getElementById('age').value, validator: (age) => { const num = parseInt(age); return !isNaN(num) && num >= 13 && num <= 200; }, message: 'Age must be between 13 and 200 years' },
            gender: { value: document.getElementById('gender').value, validator: (gender) => ['male', 'female', 'other', 'prefer-not-to-say'].includes(gender.toLowerCase()), message: 'Please select a gender' },
            password: { value: document.getElementById('password').value, validator: validatePassword, message: 'Password must meet all requirements' }
        };
        ...
        Object.keys(fields).forEach(fieldId => {
            ...
            if (!field.value || field.value.trim() === '') {
                showFieldError(fieldId, `Please enter ${fieldId === 'firstName' ? 'first name' : fieldId === 'lastName' ? 'last name' : fieldId}`);
