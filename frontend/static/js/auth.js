...
async function checkAuth() {
  try {
    ...
    if (data.authenticated) {
      ...
      trendingSection.innerHTML = `<h3>Users</h3><div class="online-users-list">Login to see online users</div>`;
      openAuthModal("login");
      stopAuthCheck();
    }
  } catch (error) {
...
function startAuthCheck() {
  authInterval = setInterval(checkAuth, 1000);
}
  ...
  try {
    ...
    if (response.ok && data.success) {
      ...
      if (data.message) {
        ...
        showAuthError(
          "Invalid credentials. Please check your email/nickname and password."
        );
      } else {
